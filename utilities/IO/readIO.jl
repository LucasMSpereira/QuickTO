...
# it in "dense" form, on which many utility functions
# are based
...
# load previous GAN models
function loadTrainedGANs(model::Symbol)
  if model == :topologyGAN
    BSON.@load datasetPath * "trainedNetworks/topologyGANgen.bson" cpuGenerator
    BSON.@load datasetPath * "trainedNetworks/topologyGANdisc.bson" cpuDiscriminator
    return (
      cpuGenerator |> gpu,
...
# read file from topologyGAN dataset
  function readTopologyGANdataset(path::String; print = false)

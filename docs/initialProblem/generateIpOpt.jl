using Makie, TopOpt, Parameters, StatProfilerHTML, NonconvexIpopt, NonconvexCore;
...
@with_kw mutable struct FEAparameters
    ...
    pos::Array{Int} = [3220, 1515, 3131] # Indices of nodes to apply concentrated force
...
function loopTO(FEAparameters)
    ...
    for i in 1:FEAparameters.quants
        ...
        model = NonconvexIpopt.Model(obj)
        addvar!(model, zeros(nels), ones(nels);
            init = x0,
            integer = fill(false, nels))
        add_ineq_constraint!(model, constr)

        # Optimization
        FEAparameters.results[i] = optimize(model, IpoptAlg(), x0, options = IpoptOptions())
        
    end

    return FEAparameters.results

end

@time result = loopTO(FEAparams);
